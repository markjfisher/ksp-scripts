{
  ...
  function exec {
    parameter wrp is 0, n is nextnode, v is n:burnvector, stT is time:seconds + n:eta - mnv_time(v:mag)/2.
  ...
  function emptyCond {
    parameter mnv. print("in empty"). return 0.
  }
  ...
  function tfTo {
    parameter tOrb, tBody. return (tOrb:hasnextpatch and tOrb:nextpatch:body = tBody).
  }
  ...
  function slopeAt {
    parameter b, t. return (sepAt(b, t + 1) - sepAt(b, t - 1)) / 2.
  }

  function sepAt {
    parameter b, t. return (positionat(ship, t) - positionat(b, t)):mag.
  }
  ...
  function mnv_time {
    ...
    list engines in es. for en in es { set t to t + en:possiblethrust. set isp to isp + en:visp. set eCnt to eCnt + 1. }
    set isp to isp / eCnt.
    return g * ship:mass * isp * (1 - constant():e^(-dV/(g*isp))) / t.
  }
